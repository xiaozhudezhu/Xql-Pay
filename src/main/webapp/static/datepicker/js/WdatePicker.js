...
(function() {
	...
	A = J();
	if ($.$wdate)
		K(A + "rest/css/datepicker/WdatePicker.css");
	U = X;
	if ($.$crossFrame) {
		try {
	...
	function J() {
		...
		for ( var B = 0; B < $.length; B++) {
			...
			_ = _.substr(0, _.toLowerCase().indexOf("rest/js/lib/wdatepicker.js"));
			A = _.lastIndexOf("/");
			if (A > 0)
				_ = _.substring(0, A + 1);
			if (_)
	...
	function I(G, _) {
		...
		function E(F, E) {
			...
			var G = [
					...
					"</script><script src=", A, "rest/js/lib/lang/", H.name,
					".js charset=", H.charset, "></script>" ];
			for ( var I = 0; I < C.length; I++)
			//	if (C[I].name == E.skin)
					G.push("<link rel=\"stylesheet\" type=\"text/css\" href=\""
							+ A + "rest/css/datepicker/datepicker.css\" charset=\"" + C[I].charset
							+ "\"/>");
			G
					.push("<script type=\"text/javascript\" src=\""
							+ A
							+ "rest/js/lib/calendar.js?\"+Math.random()+\" charset=\"gb2312\"></script>");
			G
